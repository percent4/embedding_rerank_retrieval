...
for file in os.listdir("."):
    if file.endswith("csv") and "embedding" in file:
        ...
        if len(parts) >= 2:
            ...
            # 初始化模型的hit_rate列表
            if model not in model_hit_rate_dict:
                model_hit_rate_dict[model] = []
...
# 按第二行（top-2）的hit_rate指标值进行排序
sorted_models = sorted(model_hit_rate_dict.items(), key=lambda x: x[1][1] if len(x[1]) > 1 else 0)
...
figure.update_layout(
    ...
    yaxis_range=[0, 1],  # hit_rate指标的完整取值范围0-1
...
print(f"Hit Rate统计图表已生成：embedding_{metric}.html")
print(f"包含的模型: {list(model_hit_rate_dict.keys())}")
